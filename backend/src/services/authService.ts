...
export const authService = {
  ...
  async register(input: RegisterInput) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Default permissions for the company owner — FULL access to their tenant.
      // Resource/action names MUST match the canonical set used by the route
      // guards (requirePermission) and the frontend RBAC (utils/permissions).
      const permissions = [
