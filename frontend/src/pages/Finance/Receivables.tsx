...
export default function Receivables() {
  ...
  const approve = async (id: string) => { try { await financeService.approveAR(id); toast.success('Approved'); arQ.refetch() } catch (e: any) { toast.error(e?.response?.data?.message || 'Failed') } }
  const doPay = async () => {
    setSaving(true)
    try {
